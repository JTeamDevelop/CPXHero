...
CPX.explore = function (RNG,nature,danger) {
  var R= "";
  if(RNG.bool({likelihood: danger})) { R+="D"; }
  ...
  if (nature == "overland") {
    ...
    if(R.includes("D")) { tP = [4,2,4]; }
    R+= RNG.weighted(["","S","T"],tP);
    //mod hazard probability
    if(!R.includes("D")) {
      if(R.includes("S") || R.includes("T")) { hP= [6,4] }
    }
    R+= RNG.weighted(["","H"],hP);
    //mod feature probability
    if(R.length==0) { fP = [7,3]; }
    R+= RNG.weighted(["","F"],fP);
  }
